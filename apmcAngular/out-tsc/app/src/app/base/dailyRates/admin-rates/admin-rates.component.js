...
let AdminRatesComponent = class AdminRatesComponent {
    constructor(itemService, dailyRateService, http, dateFormatter, toster, calendar, ngbCalendar, dateAdapter, modal, fb, tosterService) {
    ...
    initForm() {
        ...
        this.itemsList.forEach((item) => {
            ...
            this.dailyRates.get('dailyRateArray').push(new FormGroup({
                itemId: new FormControl(item['itemId']),
                itemName: new FormControl({
                    value: item['itemName'],
                    disabled: true,
                }),
                minPrice: new FormControl(minPrice, [Validators.required, Validators.pattern('[0-9]+')]),
                maxPrice: new FormControl(maxPrice, [Validators.required, Validators.pattern('[0-9]+')]),
                quantity: new FormControl(quantity, [Validators.required, Validators.pattern('[0-9]+')]),
                income: new FormControl({ value: income, disabled: true }, [Validators.required, Validators.pattern('[0-9]+')])
            }));
        });
        this.dataFetched = true;
    }
    get dailyRateControls() {
        return this.dailyRates.get('dailyRateArray');
    }
    //datepicker methods
    onDateSelect(dp) {
    ...
    mapperRateToItem(item) {
        const itemId = item['itemId'];
        return this.rateObj.get(itemId);
    }
    onFormSubmit(i) {
        // console.log(this.dailyRates);
        const item = { itemId: this.dailyRates.value['dailyRateArray'][i]['itemId'], itemName: this.dailyRates.value['dailyRateArray'][i]['itemName'], itemType: new ItemType(0, ''), dailyRates: [] };
        ...
        const itemRates = { rateId: 0, item: item, minPrice: minPrice, avgPrice: avgPrice, maxPrice: maxPrice, quantity: quantity, income: income, day: this.day };
...
export { AdminRatesComponent };
//# sourceMappingURL=admin-rates.component.js.map